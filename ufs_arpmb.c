	...
	if (!ret) {
		if (be32toh(ufs_arpmb_resp->bsg_reply.upiu_rsp.header.dword_1) & 0xFFFF) {
		 ...
		 be32toh(ufs_arpmb_resp->bsg_reply.upiu_rsp.header.dword_1) & 0xFF);
			...
			print_error("ARPMB OP failed %d :%d\n", ret, ufs_arpmb_resp->bsg_reply.result);
...
static int do_write_arpmb(int fd, const unsigned char *key, int input_fd,
...
{
	printf("\n Advance RPMB command usage:\n");
	printf("\n\t%s arpmb [-t] <rpmb cmd idn> [-p] <path to device>"
		" -k <path to key_file> -l <lun> -d <output/input file.\n",
		tool_name);
	printf("\n\t-t\t RPMB cmd type idn\n"
		"\t\t\t0:\tKey provision\n"
	...
	printf("\n\tExample 1 - Read 16MB of data from RPMB LUN started "
